<?php
// This cointroller will be replacy in Hexters\CoinPayment\Http\Controllers;
...
class CoinPaymentsController extends Controller {
    ...
    public function make_transaction(Request $req){
      ...
      $user = auth()->user();
      DB::table('coinpayment_transection')->insert([
        'user_id'=>$user->id,
    ...
    public function trx_info(Request $req){
      ...
      if($payment['error'] == 'ok' && (INT) $user->coinpayment_transactions()->where('payment_id', $req->result['txn_id'])->count('id') === 0){
        ...
        $lastId=DB::table('coinpayment_transection')->where('user_id',$user->id)->orderBy('id','DESC')->first();
        DB::table('coinpayment_transection')->where('id',$lastId->id)->update($saved);
